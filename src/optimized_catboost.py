"""
Optimized CatBoost model with tuned hyperparameters
"""
import pandas as pd
import numpy as np
...
# Endpoint-specific hyperparameters (tuned based on CV results)
HYPERPARAMS = {
    'LogD': {'depth': 8, 'lr': 0.03, 'iters': 1000},  # Best performer
    'KSOL': {'depth': 8, 'lr': 0.03, 'iters': 800},
    'HLM CLint': {'depth': 7, 'lr': 0.05, 'iters': 600},  # Harder task
    'MLM CLint': {'depth': 7, 'lr': 0.05, 'iters': 600},
    'Caco-2 Permeability Papp A>B': {'depth': 7, 'lr': 0.04, 'iters': 700},
    'Caco-2 Permeability Efflux': {'depth': 6, 'lr': 0.05, 'iters': 500},
    'MPPB': {'depth': 8, 'lr': 0.03, 'iters': 800},
    'MBPB': {'depth': 8, 'lr': 0.03, 'iters': 800},
    'MGMB': {'depth': 6, 'lr': 0.05, 'iters': 400},  # Small dataset
}
...
def main():
    ...
    for target in TARGETS:
        ...
        print(f"\n{'='*40}\n{target} (depth={hp['depth']}, lr={hp['lr']}, iters={hp['iters']})\n{'='*40}")
    ...
    for t in TARGETS:
        sub[t] = predictions[t]
    out_path = BASE_DIR / "submissions/optimized_catboost.csv"
